...
const AssignPass = ({ username }) => {
  ...
  const createCust = async (e) => {
    ...
    // Step 1: Query to check if a document with the same name already exists
    const nameQuery = query(customersRef, where("name", "==", cust));
    const querySnapshot = await getDocs(nameQuery);

    if (querySnapshot.size > 0) {
      // Step 2: Update the existing document
      const docId = querySnapshot.docs[0].id;
      const existingCards = querySnapshot.docs[0].data().cards || [];

      // Combine the existing cards with the new cards
      const updatedCards = existingCards.concat(cards);
      ...
      <div className="frame">
        ...
        <div className="div-5">
          <div className="card">
            ...
            <div className="div-6">
              <h2 style={{ fontSize: 20 }}>{passLeft.length}</h2>
              <p style={{ fontSize: 15 }}>Remaining Pass</p>
            </div>
          </div>
          <div className="card">
            <div className="card-tick-wrapper">
            ...
            <div className="div-6">
              <h2 style={{ fontSize: 20 }}>{userPass.length}</h2>
              <p style={{ fontSize: 15 }}>Total Pass</p>
            </div>
          </div>
        </div>
